...
document.addEventListener('click', function (e) {
  ...
  if (btn) {
    ...
    if (action === 'toggle-abstract') {
      ...
      if (expanding) {
        // Remember the CSS collapsed height so pages can override it (e.g. talks: 3 lines).
        if (!el.dataset.collapsedHeight) el.dataset.collapsedHeight = getComputedStyle(el).maxHeight;
        ...
        el.style.maxHeight = el.dataset.collapsedHeight || '5.95rem';
...
document.addEventListener('keydown', function (e) {
  if (e.key === 'Escape') {
    document.querySelectorAll('.pub-bibtex-modal.open').forEach(m => m.classList.remove('open'));
    closeVideoModal();
  }
});

// ── Video modal (talks page) ──────────────────────────────────
// Uses the privacy-enhanced youtube-nocookie.com embed.
// cc_load_policy=3 disables closed captions on start (undocumented; 1 forces them on).
let _videoModal = null;

function getVideoModal() {
  if (_videoModal) return _videoModal;
  _videoModal = document.createElement('div');
  _videoModal.className = 'video-modal';
  _videoModal.setAttribute('role', 'dialog');
  _videoModal.setAttribute('aria-modal', 'true');
  _videoModal.setAttribute('aria-label', 'Video');
  _videoModal.innerHTML = '<div class="video-modal-inner"><button class="video-modal-close" aria-label="Close">&times;</button><div class="video-modal-box"></div></div>';
  _videoModal.addEventListener('click', e => {
    if (e.target === _videoModal || e.target.closest('.video-modal-close')) closeVideoModal();
  });
  document.body.appendChild(_videoModal);
  return _videoModal;
}
...
function openVideoModal(id) {
  ...
  box.innerHTML = '<iframe src="https://www.youtube-nocookie.com/embed/' + id + '?autoplay=1&rel=0&modestbranding=1&cc_load_policy=3" title="YouTube video" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>';
...
function closeVideoModal() {
  if (!_videoModal || !_videoModal.classList.contains('open')) return;
  _videoModal.querySelector('.video-modal-box').innerHTML = ''; // stops playback
  _videoModal.classList.remove('open');
...
<footer class="site-footer">
  <div class="footer-inner">
    ...
    <div class="footer-brand">
      ...
      <p class="footer-tagline">© 2026 KE:SAI &mdash; Kyutai ELLIS Scalable Autonomous Intelligence gGmbH</p>
...
(function () {
  ...
  const isBlogPage    = !isLandingPage && !!document.querySelector('.blog-body');
  ...
  if (isBlogPage) document.documentElement.classList.remove('blog-loading');

  if (isLandingPage && showHeroAnimation) {
    const ANIM_BASE    = 200;
    const SLOGAN_START = 800;
    const REVEAL_START = 3200;

    let skipped = false;
    let tLogo, tSlogan, tReveal;
    ...
    function skipHeroAnimation() {
      ...
      if (btn) btn.remove();
      document.removeEventListener('wheel', skipHeroAnimation);
      document.removeEventListener('touchstart', skipHeroAnimation);
      document.documentElement.classList.add('hero-skip');
      heroLogo.classList.add('is-visible');
      heroSlogan.classList.add('is-visible');
      document.documentElement.classList.remove('scroll-locked');
      landingInView.forEach(el => el.classList.add('is-visible'));
      if (siteHeader) {
    ...
    if (skipBtn) skipBtn.addEventListener('click', skipHeroAnimation);
    document.addEventListener('wheel', skipHeroAnimation, { passive: true });
    document.addEventListener('touchstart', skipHeroAnimation, { passive: true });

    document.documentElement.classList.add('scroll-locked');

    tLogo = setTimeout(() => {
      heroLogo.classList.add('is-visible');
      tSlogan = setTimeout(() => heroSlogan.classList.add('is-visible'), SLOGAN_START);
      tReveal = setTimeout(() => {
        skipped = true;
        const btn = document.getElementById('skip-intro');
        if (btn) btn.remove();
